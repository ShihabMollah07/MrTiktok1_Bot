...
MONETAG_ZONE_ID = os.getenv("MONETAG_ZONE_ID", "11730069").strip()
MONETAG_SDK_NAME = os.getenv("MONETAG_SDK_NAME", f"show_{MONETAG_ZONE_ID}").strip()
PUBLIC_BASE_URL = os.getenv("PUBLIC_BASE_URL", "").strip().rstrip("/")
AD_SESSION_TTL_SECONDS = 15 * 60
AD_SESSIONS: dict[str, dict[str, Any]] = {}
...
def public_base_url() -> str:
    base = PUBLIC_BASE_URL or os.getenv("RENDER_EXTERNAL_URL", "").strip().rstrip("/")
    if not base:
        raise RuntimeError("PUBLIC_BASE_URL or RENDER_EXTERNAL_URL is required for the Monetag ad page")
    return base


def create_ad_session(chat_id: int, message_id: int, option: dict[str, Any], source_url: str) -> str:
    token = uuid.uuid4().hex
    with STATE_LOCK:
        AD_SESSIONS[token] = {"chat_id": chat_id, "message_id": message_id, "option": dict(option), "source_url": source_url, "created_at": time.time(), "used": False}
    return token


def cleanup_ad_sessions() -> None:
    now = time.time()
    with STATE_LOCK:
        for token in list(AD_SESSIONS):
            session = AD_SESSIONS[token]
            if session.get("used") or now - float(session.get("created_at", 0)) > AD_SESSION_TTL_SECONDS:
                AD_SESSIONS.pop(token, None)


def monetag_ad_url(chat_id: int, message_id: int, option: dict[str, Any], source_url: str) -> str:
    token = create_ad_session(chat_id, message_id, option, source_url)
    return f"{public_base_url()}/monetag/{quote(token)}"


def monetag_page(token: str) -> bytes:
    safe_token = html.escape(token, quote=True)
    zone = html.escape(MONETAG_ZONE_ID, quote=True)
    sdk = html.escape(MONETAG_SDK_NAME, quote=True)
    body = f'''<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Continue to Download</title>
<script src="//libtl.com/sdk.js" data-zone="{zone}" data-sdk="{sdk}"></script>
<style>body{{font-family:Arial,sans-serif;background:#111;color:#fff;display:flex;min-height:100vh;align-items:center;justify-content:center;margin:0}}.card{{max-width:420px;width:calc(100% - 32px);background:#1d1d1d;border-radius:18px;padding:28px;text-align:center;box-sizing:border-box}}button{{width:100%;padding:14px;border:0;border-radius:12px;background:#19a463;color:#fff;font-size:16px;font-weight:700;cursor:pointer}}button:disabled{{opacity:.55;cursor:not-allowed}}.small{{color:#aaa;font-size:13px;line-height:1.5}}</style></head><body>
<div class="card"><h2>📢 One quick step</h2><p>Ad is loading. After it loads, tap the button below to continue.</p><p class="small">You do not need to click an advertisement.</p><button id="continue" onclick="unlock()">Continue to Download</button><p id="status" class="small"></p></div>
<script>async function unlock(){{const b=document.getElementById('continue'),s=document.getElementById('status');b.disabled=true;s.textContent='Processing…';try{{const r=await fetch('/monetag/unlock?token={safe_token}',{{cache:'no-store'}});const data=await r.json();if(!r.ok||!data.ok)throw new Error(data.message||'Unable to continue');s.textContent='Done. Go back to Telegram — your download options are ready.';b.textContent='✓ Unlocked';}}catch(e){{b.disabled=false;s.textContent=e.message||'Please try again.';}}}}</script></body></html>'''
    return body.encode("utf-8")


def handle_ad_unlock(token: str) -> tuple[bool, str]:
    cleanup_ad_sessions()
    with STATE_LOCK:
        session = AD_SESSIONS.get(token)
        if not session or session.get("used"):
            return False, "This download link has expired or was already used."
        session["used"] = True
        chat_id = int(session["chat_id"]); message_id = int(session["message_id"])
        option = dict(session["option"]); source_url = str(session["source_url"])
        item = DOWNLOAD_OPTIONS.get(chat_id)
        if not item:
            return False, "Download session expired. Please send the TikTok URL again."
        item["selected"] = option
        title = item.get("title", option.get("title", "TikTok video"))
    option["title"] = title; option["source_url"] = source_url
    CHAT_MODES[chat_id] = "format"
    edit_message(chat_id, message_id, f"<b>✅ Ad step completed</b>\n\n{escape(option['label'])}\n\nএখন output format নির্বাচন করুন:", reply_markup=format_keyboard())
...
def process_callback(callback: dict[str, Any]) -> None:
    ...
    if data == "home":
        ...
        try:
            ad_url = monetag_ad_url(chat_id, message_id, option, item["source_url"])
        except Exception as error:
            log("Monetag gate URL creation failed: %s", error)
            edit_message(chat_id, message_id, "Monetag ad page configure করা হয়নি। PUBLIC_BASE_URL সেট করুন।", reply_markup=quality_keyboard(item["options"]))
            return
        CHAT_MODES[chat_id] = "ad_gate"
        edit_message(
            chat_id, message_id,
            f"<b>🎚️ {escape(option['label'])}</b>\n\nডাউনলোড চালু করতে নিচের Ad/Continue page খুলুন।",
            reply_markup=inline_keyboard([
                [{"text": "📢 Open Ad & Continue", "url": ad_url, "style": "primary"}],
                [{"text": "↩️ Back to quality", "callback_data": "back:quality", "style": "primary"}],
                [{"text": "✖️ Cancel", "callback_data": "cancel", "style": "danger"}],
            ]),
        )
...
class HealthHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:
        ...
        if parsed.path.startswith("/monetag/") and parsed.path != "/monetag/unlock":
            token = parsed.path.rsplit("/", 1)[-1]
            payload = monetag_page(token)
            self.send_response(200)
            self.send_header("Content-Type", "text/html; charset=utf-8")
            self.send_header("Cache-Control", "no-store")
            self.send_header("Content-Length", str(len(payload)))
            self.end_headers(); self.wfile.write(payload); return
        if parsed.path == "/monetag/unlock":
            token = parse_qs(parsed.query).get("token", [""])[0]
            ok, message = handle_ad_unlock(token)
